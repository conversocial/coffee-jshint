...
var CoffeeReactTransform, CoffeeScript, addPathToErrors, buildTrueObj, createInternalError, defaultOptions, errorsToSkip, formatErrors, fs, hint, hintFiles, jshint, _;
...
hint = function(coffeeSource, options, globals) {
  ...
  if (jshint(js, options, globals)) {
    ...
    return [createInternalError("jshint didn't pass but returned no errors")];
  } else {
...
createInternalError = function(path, errString) {
  return {
    path: path,
    line: '??',
    character: '??',
...
addPathToErrors = function(path, errors) {
  var error, _i, _len;
